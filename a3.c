...
void freeList(NODE* list[], int list_len);
...
double find_avg_stage_1(NODE* job[], int *num_jobs_p, int *num_machines_p);
...
void read_in_instance(int *num_machines_p, int *num_jobs_p) {
    ...
    double a1 = job[*num_jobs_p - 1]->next->data;
    
    double avg_stage_1 = find_avg_stage_1(job, num_jobs_p, num_machines_p);
    
    double L1 = (a1 > avg_stage_1)? a1:avg_stage_1;

    double L2 = johnson(job, 0, 0, *num_jobs_p - 1);
    ...
    printf("%f", r);
    freeList(job, *num_jobs_p);
    return;
}

double find_avg_stage_1(NODE* job[], int *num_jobs_p, int *num_machines_p) {
    double sum_stage_1 = 0;
    for (int i = 0; i < *num_jobs_p; i++) {
        sum_stage_1 += job[i]->next->data;
    }
    double avg_stage_1 = sum_stage_1 / (*num_machines_p);
    return avg_stage_1;
}
    ...
    */
    ...
    for (int m = 1; m <= 20; m++) {
        for (int n = 10; n <= 200; n+=10) {
            for (int t2 = 10; t2 <= 100; t2+=10) {
                // generate 100 instances containing information above
                for (int k = 1; k <= 100; k++) {
                    *num_machines_p = m;
                    *num_jobs_p = n;
                    t_1 = 1;
                    t_2 = t2;
                    char file_name[35];
                    sprintf(file_name, "instance%d_%d_1_%d_%d.txt", *num_machines_p, *num_jobs_p, t_2, k);
                    FILE *fp;
                    ...
                    for (int j = 0; j < *num_jobs_p; j++) {
                        for (int k = 0; k < 3; k++) {
                            fprintf(fp, "%d ", (rand() % (t_2 - t_1 + 1)) + t_1);
                        }
                        fprintf(fp, "\n");
                    }
                    fclose(fp);
...
int lpt(NODE* job[], int t, int str, int end, int *num_machines_p) {
    ...
    */
    ...
    freeList(machinesA, *num_machines_p+1);
    return (T-t);
}
...
int johnson(NODE* job[], int t, int str, int end) {
    ...
    */
    ...
    //printf("The job processing time interval is [%d, %d], and the makespan is %d.\n\n\n", t, T, T-t);
    //freeList(johnson_job, end-str+1);
    freeList(machinesbc, 2);
    return (T-t);
}
